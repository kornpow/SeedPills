...
//   - pill height    3.00mm = 15 x 0.2mm layers
//   - relief depth   0.60mm =  3 x 0.2mm layers
//   - border width   0.90mm =  2 x 0.45mm perimeter lines
//   - interlocks     0.50mm =  1 x 0.5mm extrusion line (snaps by hand)
...
height = 3.0;   // 15 layers at 0.2mm
deep = 0.6;     // relief depth: 3 layers at 0.2mm
border_ = 0.9;  // rim around the relief: 2 perimeters at 0.4mm nozzle
...
usable = [bed[0] - 2 * bed_margin - exclusion[0],
...
module pill(front_word, back_word, x, y) {
    ...
    union() {
        difference() {
            pill_shape();
            // Carve the relief pocket, plus letter-shaped pockets so the
            // letters sit recessed without coplanar-surface artifacts.
            // An empty word means a blank slot (past the end of the list):
            // skip the pocket entirely so the face stays flat.
            if (front_word != "")
                for (side = sides)
                    rotate([0, 180 * side, 0])
                        translate([0, 0, height / 2 - deep]) {
                            linear_extrude(deep + 0.01)
                                pill_outline(inset = border);
                            translate([0, 0, -0.02])
