// SeedPills: 3D-printable BIP39 word pills, tuned for a 0.2mm nozzle (Bambu P1S).
//
// All sizes are in millimeters. Defaults below are chosen so that every small
// feature is an integer number of 0.2mm extrusion lines or 0.12mm layers:
//   - pill height    3.00mm = 25 x 0.12mm layers
//   - relief depth   0.48mm =  4 x 0.12mm layers
//   - border width   0.80mm =  2 x 0.4mm walls (2 perimeters at 0.2mm nozzle)
//   - interlocks     0.44mm =  2 x 0.22mm extrusion lines
//   - gap between pills is bridged by the interlocks, keeping pills ordered
//     until you snap them apart

$fa = 4;    // smooth pill ends (was 10)
$fs = 0.2;  // curve resolution matched to the nozzle (was 1)

// Grid
first = 0;    // index of the first word in this grid (0 = "ABAN")
columns = 8;  // pills per row
rows = 16;    // pills per column (8x16 = 128 pills, 256 words)

// Pill
width = 18.5;   // overall pill length
lenght = 7.5;   // overall pill width
height = 3.0;   // 25 layers at 0.12mm
deep = 0.48;    // relief depth: 4 layers at 0.12mm
border_ = 0.8;  // rim around the relief: 2 perimeters at 0.2mm nozzle

// Font
font = "Fredoka";   // install Fredoka (Google Fonts) for the intended look
font_size = 3.5;

// Spacing / interlocks
spacing = 0.5;            // gap between pills, bridged by the interlocks
interlock = 0.44;         // 2 extrusion lines: printable but snaps by hand
...
// A zero border would leave a paper-thin gap; nudge it negative instead.
border = (border_ == 0) ? -0.1 : border_;

module pill_shape() {
    cube([width - lenght, lenght, height], center = true);
    for (end = [-1, 1])
        translate([end * (width - lenght) / 2, 0, 0])
...
module pill(front_word, back_word, x, y) {
    union() {
        difference() {
            pill_shape();
            // Carve the relief pocket into both faces
            for (side = [0:1])
                rotate([0, 180 * side, 0])
                    translate([0, 0, height / 2 - deep]) {
                        linear_extrude(deep + 0.01)
                            offset(delta = -border)
                                square([width, lenght], center = true);
                        // Letter-shaped pockets so the letters can sit
                        // recessed without coplanar-surface artifacts
                        translate([0, 0, -0.02])
                            linear_extrude(deep + 0.02)
                                text(side == 0 ? front_word : back_word,
                                     size = font_size, font = font,
                                     halign = "center", valign = "center");
                    }
        }

        // Raised lettering, bottom of the letters flush with the relief floor
        for (side = [0:1])
            rotate([0, 180 * side, 0])
                translate([0, 0, height / 2 - deep])
                    linear_extrude(deep)
        ...
        // Interlocks: mid-height bars bridging the gap to the previous pill
        if (x > 0)
...
for (i = [0:columns * rows - 1]) {
    ...
    translate([x * (width + spacing), y * -(lenght + spacing), 0])
        pill(words[first + i], words[first + i + 1024], x, y);
}
